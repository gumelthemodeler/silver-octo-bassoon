...
function AdminTab.Init(parentFrame)
	...
	-- [[ COLUMN 1: ATTRIBUTES ]]
	-- FIX: Changed to ScrollingFrame and enforced LayoutOrder
	local Col1 = Instance.new("ScrollingFrame", MainFrame)
	Col1.Size = UDim2.new(0.3, 0, 0.75, 0); Col1.Position = UDim2.new(0.025, 0, 0.18, 0); Col1.BackgroundColor3 = Colors.Panel
	Col1.ScrollBarThickness = 2; Col1.BorderSizePixel = 0; Col1.AutomaticCanvasSize = Enum.AutomaticSize.Y
	Instance.new("UICorner", Col1).CornerRadius = UDim.new(0, 8); Instance.new("UIStroke", Col1).Color = Colors.Border

	local C1List = Instance.new("UIListLayout", Col1); C1List.Padding = UDim.new(0, 10); C1List.HorizontalAlignment = Enum.HorizontalAlignment.Center
	C1List.SortOrder = Enum.SortOrder.LayoutOrder
	local C1Pad = Instance.new("UIPadding", Col1); C1Pad.PaddingTop = UDim.new(0, 10); C1Pad.PaddingBottom = UDim.new(0, 10)
	...
	-- [[ COLUMN 2: MACROS ]]
	-- FIX: Changed to ScrollingFrame and enforced LayoutOrder
	local Col2 = Instance.new("ScrollingFrame", MainFrame)
	Col2.Size = UDim2.new(0.3, 0, 0.75, 0); Col2.Position = UDim2.new(0.35, 0, 0.18, 0); Col2.BackgroundColor3 = Colors.Panel
	Col2.ScrollBarThickness = 2; Col2.BorderSizePixel = 0; Col2.AutomaticCanvasSize = Enum.AutomaticSize.Y
	...
	C2List.SortOrder = Enum.SortOrder.LayoutOrder
	local C2Pad = Instance.new("UIPadding", Col2); C2Pad.PaddingTop = UDim.new(0, 10); C2Pad.PaddingBottom = UDim.new(0, 10)
	...
	local function CreateMacroBtn(parent, text, cmd, args, color)
		...
		btn.LayoutOrder = c2Order; c2Order += 1
	...
	local wipeBtn = CreateMacroBtn(Col2, "WIPE TARGET (KEEPS ITEMS)", "WipePlayer", nil, Color3.fromRGB(150, 40, 40))
	wipeBtn.UIStroke.Color = Color3.fromRGB(200, 50, 50)
	local wipeConfirm = false

	-- Overwrite default click for double-click confirmation
	...
	local globalBtn = CreateMacroBtn(Col2, "GLOBAL ROLLBACK (ALL SERVERS)", "GlobalRollback", nil, Color3.fromRGB(100, 0, 0))
	globalBtn.UIStroke.Color = Color3.fromRGB(255, 0, 0); globalBtn.UIStroke.Thickness = 2
	...
	local function PopulateDictionary()
		local allItems = {}
		-- [[ THE FIX: Exclude any item containing "Gift" from the Spawner List! ]]
		for k, _ in pairs(ItemData.Equipment) do 
			if not string.find(string.lower(k), "gift") then table.insert(allItems, k) end
		end
		for k, _ in pairs(ItemData.Consumables) do 
			if not string.find(string.lower(k), "gift") then table.insert(allItems, k) end
		end
		table.sort(allItems)
