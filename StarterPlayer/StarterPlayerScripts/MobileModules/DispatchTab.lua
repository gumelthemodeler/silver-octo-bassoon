...
local FlavorTexts = { "Scavenging ruins...", "Hiding from a Titan...", "Mapping territory...", "Setting up a camp...", "Resting horses...", "Found a flare...", "Looting an outpost..." }
...
function DispatchTab.Init(parentFrame)
	...
	local mainLayout = Instance.new("UIListLayout", MainFrame); mainLayout.SortOrder = Enum.SortOrder.LayoutOrder; mainLayout.Padding = UDim.new(0, 10)

	local Title = Instance.new("TextLabel", MainFrame)
	Title.Size = UDim2.new(1, 0, 0, 30); Title.BackgroundTransparency = 1; Title.Font = Enum.Font.GothamBlack; Title.TextColor3 = Color3.fromRGB(255, 215, 100); Title.TextSize = 20; Title.Text = "EXPEDITIONS (AFK)"; Title.LayoutOrder = 1

	-- [[ MOBILE MAP RECONSTRUCTION ]]
	MapPanel = Instance.new("Frame", MainFrame)
	MapPanel.Size = UDim2.new(1, 0, 0, 240); MapPanel.BackgroundColor3 = Color3.fromRGB(15, 18, 22); MapPanel.ClipsDescendants = true; MapPanel.LayoutOrder = 2
	Instance.new("UICorner", MapPanel).CornerRadius = UDim.new(0, 8); Instance.new("UIStroke", MapPanel).Color = Color3.fromRGB(60, 70, 80); MapPanel.UIStroke.Thickness = 2
	...
	local function DrawWall(name, scale, color, thick)
		...
		-- Fixed: Added the Wall TextLabels back to the mobile version
	...
	TopDashboard.Size = UDim2.new(1, 0, 0, 35); TopDashboard.BackgroundColor3 = Color3.fromRGB(20, 20, 25); TopDashboard.BackgroundTransparency = 0.4; TopDashboard.ZIndex = 20
	Instance.new("UIStroke", TopDashboard).Color = Color3.fromRGB(100, 80, 40)

	TotalDewsLbl = Instance.new("TextLabel", TopDashboard); TotalDewsLbl.Size = UDim2.new(0.3, 0, 1, 0); TotalDewsLbl.Position = UDim2.new(0.05, 0, 0, 0); TotalDewsLbl.BackgroundTransparency = 1; TotalDewsLbl.Font = Enum.Font.GothamBold; TotalDewsLbl.TextColor3 = Color3.fromRGB(150, 220, 255); TotalDewsLbl.TextSize = 11; TotalDewsLbl.TextXAlignment = Enum.TextXAlignment.Left; TotalDewsLbl.Text = "0 DEWS"
	TotalXPLbl = Instance.new("TextLabel", TopDashboard); TotalXPLbl.Size = UDim2.new(0.3, 0, 1, 0); TotalXPLbl.Position = UDim2.new(0.4, 0, 0, 0); TotalXPLbl.BackgroundTransparency = 1; TotalXPLbl.Font = Enum.Font.GothamBold; TotalXPLbl.TextColor3 = Color3.fromRGB(100, 255, 100); TotalXPLbl.TextSize = 11; TotalXPLbl.TextXAlignment = Enum.TextXAlignment.Left; TotalXPLbl.Text = "0 XP"

	local RecallAllBtn = Instance.new("TextButton", TopDashboard)
	RecallAllBtn.Size = UDim2.new(0.25, 0, 0.7, 0); RecallAllBtn.AnchorPoint = Vector2.new(1, 0.5); RecallAllBtn.Position = UDim2.new(0.98, 0, 0.5, 0); RecallAllBtn.BackgroundColor3 = Color3.fromRGB(160, 60, 60); RecallAllBtn.Font = Enum.Font.GothamBlack; RecallAllBtn.TextColor3 = Color3.new(1,1,1); RecallAllBtn.TextSize = 9; RecallAllBtn.Text = "RECALL ALL"; RecallAllBtn.ZIndex = 21; Instance.new("UICorner", RecallAllBtn).CornerRadius = UDim.new(0, 4)
	...
	StatusOverlay.Size = UDim2.new(0.8, 0, 0.5, 0); StatusOverlay.Position = UDim2.new(0.1, 0, 0.25, 0); StatusOverlay.BackgroundColor3 = Color3.fromRGB(20, 20, 25); StatusOverlay.Visible = false; StatusOverlay.ZIndex = 30; Instance.new("UICorner", StatusOverlay).CornerRadius = UDim.new(0, 8); Instance.new("UIStroke", StatusOverlay).Color = Color3.fromRGB(150, 200, 150)
	local sName = Instance.new("TextLabel", StatusOverlay); sName.Size = UDim2.new(1, 0, 0, 25); sName.BackgroundTransparency = 1; sName.Font = Enum.Font.GothamBlack; sName.TextColor3 = Color3.fromRGB(255, 255, 255); sName.TextSize = 14; sName.Text = "ALLY NAME"; sName.ZIndex = 31
	local sLoot = Instance.new("TextLabel", StatusOverlay); sLoot.Size = UDim2.new(1, 0, 0, 25); sLoot.Position = UDim2.new(0, 0, 0, 25); sLoot.BackgroundTransparency = 1; sLoot.Font = Enum.Font.GothamBold; sLoot.TextColor3 = Color3.fromRGB(255, 215, 100); sLoot.TextSize = 12; sLoot.Text = "Loot: 0 Dews | 0 XP"; sLoot.ZIndex = 31
	local sRecallBtn = Instance.new("TextButton", StatusOverlay); sRecallBtn.Size = UDim2.new(0.8, 0, 0, 30); sRecallBtn.Position = UDim2.new(0.1, 0, 1, -40); sRecallBtn.BackgroundColor3 = Color3.fromRGB(150, 60, 60); sRecallBtn.Font = Enum.Font.GothamBlack; sRecallBtn.TextColor3 = Color3.new(1,1,1); sRecallBtn.TextSize = 12; sRecallBtn.Text = "RECALL & CLAIM"; sRecallBtn.ZIndex = 31; Instance.new("UICorner", sRecallBtn).CornerRadius = UDim.new(0, 4)
	...
	RosterPanel.Size = UDim2.new(1, 0, 0, 0); RosterPanel.AutomaticSize = Enum.AutomaticSize.Y; RosterPanel.BackgroundColor3 = Color3.fromRGB(20, 20, 25); RosterPanel.LayoutOrder = 3
	...
	RosterTitle = Instance.new("TextLabel", HeaderArea); RosterTitle.Size = UDim2.new(0.7, 0, 1, 0); RosterTitle.Position = UDim2.new(0.05, 0, 0, 0); RosterTitle.BackgroundTransparency = 1; RosterTitle.Font = Enum.Font.GothamBlack; RosterTitle.TextColor3 = Color3.fromRGB(255, 255, 255); RosterTitle.TextSize = 14; RosterTitle.TextXAlignment = Enum.TextXAlignment.Left; RosterTitle.Text = "ROSTER"

	local AddSlotBtn = Instance.new("TextButton", HeaderArea); AddSlotBtn.Size = UDim2.new(0, 25, 0, 25); AddSlotBtn.AnchorPoint = Vector2.new(1, 0.5); AddSlotBtn.Position = UDim2.new(0.95, 0, 0.5, 0); AddSlotBtn.BackgroundColor3 = Color3.fromRGB(40, 120, 40); Instance.new("UICorner", AddSlotBtn).CornerRadius = UDim.new(0, 6); AddSlotBtn.Font = Enum.Font.GothamBlack; AddSlotBtn.TextColor3 = Color3.new(1,1,1); AddSlotBtn.TextSize = 18; AddSlotBtn.Text = "+"
	...
	RosterList.Size = UDim2.new(1, -20, 0, 0); RosterList.Position = UDim2.new(0, 10, 0, 40); RosterList.BackgroundTransparency = 1; RosterList.AutomaticSize = Enum.AutomaticSize.Y
	...
	local function RenderRoster()
		...
		for _, ally in ipairs(Allies) do
			...
			local row = Instance.new("Frame", RosterList); row.Size = UDim2.new(1, 0, 0, 65); row.BackgroundColor3 = Color3.fromRGB(25, 25, 30); Instance.new("UICorner", row).CornerRadius = UDim.new(0, 6); Instance.new("UIStroke", row).Color = ally.Color
			local lvl = allyLevels[ally.Name] or 1; local lvlMult = 1 + ((lvl - 1) * 0.20)

			local nLbl = Instance.new("TextLabel", row); nLbl.Size = UDim2.new(0.6, 0, 0, 20); nLbl.Position = UDim2.new(0, 10, 0, 5); nLbl.BackgroundTransparency = 1; nLbl.Font = Enum.Font.GothamBold; nLbl.TextColor3 = ally.Color; nLbl.TextSize = 13; nLbl.TextXAlignment = Enum.TextXAlignment.Left; nLbl.Text = ally.Name
			local lvlLbl = Instance.new("TextLabel", row); lvlLbl.Size = UDim2.new(0.3, 0, 0, 20); lvlLbl.AnchorPoint = Vector2.new(1, 0); lvlLbl.Position = UDim2.new(1, -10, 0, 5); lvlLbl.BackgroundTransparency = 1; lvlLbl.Font = Enum.Font.GothamBlack; lvlLbl.TextColor3 = Color3.fromRGB(255, 215, 100); lvlLbl.TextSize = 11; lvlLbl.TextXAlignment = Enum.TextXAlignment.Right; lvlLbl.Text = "Lvl " .. lvl .. " (".. math.floor(lvlMult*100) .."%)"; if not isUnlocked then lvlLbl.Visible = false end
			local sLbl = Instance.new("TextLabel", row); sLbl.Size = UDim2.new(1, -20, 0, 15); sLbl.Position = UDim2.new(0, 10, 0, 25); sLbl.BackgroundTransparency = 1; sLbl.Font = Enum.Font.GothamMedium; sLbl.TextColor3 = Color3.fromRGB(150, 150, 150); sLbl.TextSize = 10; sLbl.TextXAlignment = Enum.TextXAlignment.Left
			local btn = Instance.new("TextButton", row); btn.Size = UDim2.new(0.45, 0, 0, 20); btn.Position = UDim2.new(0, 10, 1, -25); btn.Font = Enum.Font.GothamBold; btn.TextColor3 = Color3.new(1,1,1); btn.TextSize = 10; Instance.new("UICorner", btn).CornerRadius = UDim.new(0, 4)
			local upgBtn = Instance.new("TextButton", row); upgBtn.Size = UDim2.new(0.45, 0, 0, 20); upgBtn.AnchorPoint = Vector2.new(1, 0); upgBtn.Position = UDim2.new(1, -10, 1, -25); upgBtn.BackgroundColor3 = Color3.fromRGB(150, 100, 30); upgBtn.Font = Enum.Font.GothamBold; upgBtn.TextColor3 = Color3.new(1,1,1); upgBtn.TextSize = 10; upgBtn.Visible = false; Instance.new("UICorner", upgBtn).CornerRadius = UDim.new(0, 4)

			if not isUnlocked then
				if curPart < ally.ReqPart then sLbl.Text = "Req Part " .. ally.ReqPart; btn.Text = "LOCKED"; btn.BackgroundColor3 = Color3.fromRGB(60, 60, 65)
				else sLbl.Text = "Available!"; btn.Text = "UNLOCK (" .. (ally.Cost or 0) .. ")"; btn.BackgroundColor3 = Color3.fromRGB(200, 150, 50); btn.MouseButton1Click:Connect(function() Network:WaitForChild("DispatchAction"):FireServer("UnlockAlly", ally.Name) end) end
			else
				upgBtn.Visible = true
				if lvl >= 10 then upgBtn.Text = "MAX LVL"; upgBtn.BackgroundColor3 = Color3.fromRGB(60, 60, 60) else upgBtn.Text = "UPG (" .. (lvl * 5000) .. ")"; upgBtn.MouseButton1Click:Connect(function() Network:WaitForChild("DispatchAction"):FireServer("UpgradeAlly", ally.Name) end) end
				if dData[ally.Name] then local mins = math.floor((os.time() - dData[ally.Name].StartTime) / 60); sLbl.Text = "Exploring... (" .. mins .. " mins)"; btn.Text = "RECALL"; btn.BackgroundColor3 = Color3.fromRGB(160, 60, 60); btn.MouseButton1Click:Connect(function() Network:WaitForChild("DispatchAction"):FireServer("Recall", ally.Name) end)
				else sLbl.Text = "Ready."; btn.Text = "DEPLOY"; btn.BackgroundColor3 = Color3.fromRGB(60, 100, 160); btn.MouseButton1Click:Connect(function() Network:WaitForChild("DispatchAction"):FireServer("Deploy", ally.Name) end) end
			end
		end
	end
	...
	local function SyncAvatars()
		...
		for name, info in pairs(dData) do
			if not activeAvatars[name] then
				local charColor = Color3.new(1,1,1); local allyIcon = ""
				for _, a in ipairs(Allies) do if a.Name == name then charColor = a.Color; allyIcon = a.Icon break end end
				...
				card.Size = UDim2.new(0, 30, 0, 30); card.Position = UDim2.new(0.5, 0, 1.1, 0); card.AnchorPoint = Vector2.new(0.5, 0.5); card.BackgroundColor3 = Color3.fromRGB(30, 30, 35); card.Text = ""; card.ZIndex = 5
				Instance.new("UICorner", card).CornerRadius = UDim.new(0, 8); Instance.new("UIStroke", card).Color = charColor; card.UIStroke.Thickness = 2; card:SetAttribute("NodeIndex", chosenIdx)
				...
				card.MouseButton1Click:Connect(function()
					...
					local mins = math.floor((os.time() - info.StartTime) / 60)
			...
			if MainFrame.Visible then
				...
				if StatusOverlay.Visible and selectedAlly then
					if dData[selectedAlly] then
						local mins = math.floor((os.time() - dData[selectedAlly].StartTime) / 60)
