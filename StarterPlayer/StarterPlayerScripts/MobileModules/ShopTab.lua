...
function ShopTab.Init(parentFrame, tooltipMgr)
	...
	Title.Size = UDim2.new(0.95, 0, 0, 35); Title.BackgroundTransparency = 1; Title.Font = Enum.Font.GothamBlack; Title.TextColor3 = Color3.fromRGB(255, 255, 255); Title.TextSize = 20; Title.Text = "MARKETPLACE & SUPPLY"; Title.TextXAlignment = Enum.TextXAlignment.Center
	ApplyGradient(Title, Color3.fromRGB(150, 200, 255), Color3.fromRGB(50, 150, 255))
	Title.LayoutOrder = 0

	CodePanel = Instance.new("Frame", MainFrame)
	CodePanel.Size = UDim2.new(0.95, 0, 0, 120); CodePanel.BackgroundColor3 = Color3.fromRGB(20, 20, 25); CodePanel.LayoutOrder = 1
	...
	cTitle.Size = UDim2.new(0.9, 0, 0, 20); cTitle.BackgroundTransparency = 1; cTitle.Font = Enum.Font.GothamBlack; cTitle.TextColor3 = Color3.fromRGB(200, 200, 200); cTitle.TextSize = 13; cTitle.TextXAlignment = Enum.TextXAlignment.Center; cTitle.Text = "ENTER PROMO CODE:"

	local cInput = Instance.new("TextBox", CodePanel)
	cInput.Size = UDim2.new(0.9, 0, 0, 35); cInput.BackgroundColor3 = Color3.fromRGB(15, 15, 18); cInput.Font = Enum.Font.GothamBold; cInput.TextColor3 = Color3.fromRGB(255, 255, 255); cInput.TextSize = 12; cInput.PlaceholderText = "Type code here..."
	...
	cBtn.Size = UDim2.new(0.9, 0, 0, 35); cBtn.Font = Enum.Font.GothamBlack; cBtn.TextColor3 = Color3.fromRGB(255, 255, 255); cBtn.TextSize = 13; cBtn.Text = "REDEEM"
	...
	SupplyPanel.Size = UDim2.new(0.95, 0, 0, 0); SupplyPanel.AutomaticSize = Enum.AutomaticSize.Y; SupplyPanel.BackgroundColor3 = Color3.fromRGB(20, 20, 25)
	Instance.new("UICorner", SupplyPanel).CornerRadius = UDim.new(0, 8); Instance.new("UIStroke", SupplyPanel).Color = Color3.fromRGB(80, 80, 90); SupplyPanel.LayoutOrder = 2

	local sListLayout = Instance.new("UIListLayout", SupplyPanel); sListLayout.Padding = UDim.new(0, 10); sListLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center; sListLayout.SortOrder = Enum.SortOrder.LayoutOrder
	local sPad = Instance.new("UIPadding", SupplyPanel); sPad.PaddingTop = UDim.new(0, 10); sPad.PaddingBottom = UDim.new(0, 15)
	...
	RRArea.Size = UDim2.new(1, 0, 0, 40); RRArea.BackgroundTransparency = 1
	...
	RRBtn.Size = UDim2.new(0.48, 0, 1, 0); RRBtn.Font = Enum.Font.GothamBold; RRBtn.TextColor3 = Color3.fromRGB(255,255,255); RRBtn.TextSize = 10; RRBtn.Text = "RESTOCK (15 R$)"
	...
	ShopGrid.Size = UDim2.new(1, -10, 0, 0); ShopGrid.AutomaticSize = Enum.AutomaticSize.Y
	ShopGrid.BackgroundTransparency = 1; ShopGrid.BorderSizePixel = 0; ShopGrid.LayoutOrder = 2
	local sgLayout = Instance.new("UIListLayout", ShopGrid); sgLayout.Padding = UDim.new(0, 8)
	...
	local function FetchAndRenderShop()
		...
		for _, item in ipairs(currentShopData.Items) do
			...
			row.Size = UDim2.new(1, 0, 0, 75); row.BackgroundColor3 = Color3.fromRGB(22, 22, 28); Instance.new("UICorner", row).CornerRadius = UDim.new(0, 6)
			...
			local tagBox = Instance.new("Frame", row); tagBox.Size = UDim2.new(0, 14, 0, 14); tagBox.Position = UDim2.new(0, 6, 0, 8); tagBox.BackgroundColor3 = rarityRGB; Instance.new("UICorner", tagBox).CornerRadius = UDim.new(0, 4)
			local tagTxt = Instance.new("TextLabel", tagBox); tagTxt.Size = UDim2.new(1, 0, 1, 0); tagTxt.BackgroundTransparency = 1; tagTxt.Font = Enum.Font.GothamBlack; tagTxt.TextColor3 = Color3.new(0,0,0); tagTxt.TextSize = 9; tagTxt.Text = string.sub(rarityTag, 1, 1)

			local nLbl = Instance.new("TextLabel", row)
			nLbl.Size = UDim2.new(1, -25, 0, 30); nLbl.Position = UDim2.new(0, 25, 0, 5); nLbl.BackgroundTransparency = 1; nLbl.Font = Enum.Font.GothamBold; nLbl.TextColor3 = Color3.fromRGB(255,255,255); nLbl.TextXAlignment = Enum.TextXAlignment.Left; nLbl.RichText = true; nLbl.TextSize = 11; nLbl.TextWrapped = true

			local bonusStr = ""
			if iData and iData.Bonus then
				local bList = {}
				for k, v in pairs(iData.Bonus) do table.insert(bList, "+"..v.." "..string.sub(k, 1, 3):upper()) end
				bonusStr = "\n<font color='#55FF55' size='9'>" .. table.concat(bList, " | ") .. "</font>"
			end

			nLbl.Text = "<b><font color='" .. cColor .. "'>" .. item.Name .. "</font></b>" .. bonusStr

			local cLbl = Instance.new("TextLabel", row); cLbl.Size = UDim2.new(0.5, 0, 0, 20); cLbl.Position = UDim2.new(0, 10, 1, -25); cLbl.BackgroundTransparency = 1; cLbl.Font = Enum.Font.GothamMedium; cLbl.TextColor3 = Color3.fromRGB(150, 255, 150); cLbl.TextXAlignment = Enum.TextXAlignment.Left; cLbl.TextSize = 10
			cLbl.Text = "Cost: " .. item.Cost .. " Dews"

			local bBtn = Instance.new("TextButton", row); bBtn.Size = UDim2.new(0.3, 0, 0, 30); bBtn.AnchorPoint = Vector2.new(1, 0); bBtn.Position = UDim2.new(1, -10, 1, -35); 
			...
			if item.SoldOut then
				...
				bBtn.MouseButton1Click:Connect(function()
					...
					if player.leaderstats and player.leaderstats:FindFirstChild("Dews") and player.leaderstats.Dews.Value >= item.Cost then
						item.SoldOut = true 
						Network.ShopAction:FireServer("BuyItem", item.Name)
	...
	PremiumPanel.Size = UDim2.new(0.95, 0, 0, 350); PremiumPanel.BackgroundColor3 = Color3.fromRGB(20, 20, 25); PremiumPanel.LayoutOrder = 3
	...
	PTitle.Size = UDim2.new(1, 0, 0, 25); PTitle.BackgroundTransparency = 1; PTitle.Font = Enum.Font.GothamBlack; PTitle.TextColor3 = Color3.fromRGB(255, 215, 100); PTitle.TextSize = 14; PTitle.Text = "PREMIUM STORE"; PTitle.LayoutOrder = 1

	local PremList = Instance.new("ScrollingFrame", PremiumPanel)
	PremList.Size = UDim2.new(1, -10, 1, -40); PremList.BackgroundTransparency = 1; PremList.LayoutOrder = 2; PremList.ScrollBarThickness = 4; PremList.BorderSizePixel = 0
	local plLayout = Instance.new("UIListLayout", PremList); plLayout.Padding = UDim.new(0, 8)
	...
	for _, gp in ipairs(ItemData.Gamepasses) do
		...
		row.Size = UDim2.new(1, -5, 0, 90); row.BackgroundColor3 = Color3.fromRGB(22, 22, 28)
		...
		local rTitle = Instance.new("TextLabel", row); rTitle.Size = UDim2.new(1, -15, 0, 18); rTitle.Position = UDim2.new(0, 10, 0, 5); rTitle.BackgroundTransparency = 1; rTitle.Font = Enum.Font.GothamBlack; rTitle.TextColor3 = Color3.fromRGB(255, 215, 100); rTitle.TextSize = 12; rTitle.TextXAlignment = Enum.TextXAlignment.Left; rTitle.Text = gp.Name
		local rDesc = Instance.new("TextLabel", row); rDesc.Size = UDim2.new(1, -15, 0, 30); rDesc.Position = UDim2.new(0, 10, 0, 25); rDesc.BackgroundTransparency = 1; rDesc.Font = Enum.Font.GothamMedium; rDesc.TextColor3 = Color3.fromRGB(200, 200, 200); rDesc.TextSize = 10; rDesc.TextWrapped = true; rDesc.TextXAlignment = Enum.TextXAlignment.Left; rDesc.Text = gp.Desc

		local btnArea = Instance.new("Frame", row)
		btnArea.Size = UDim2.new(1, -15, 0, 25); btnArea.Position = UDim2.new(0, 10, 1, -30); btnArea.BackgroundTransparency = 1
		...
		buyBtn.Size = UDim2.new(0.48, 0, 1, 0); buyBtn.Font = Enum.Font.GothamBold; buyBtn.TextColor3 = Color3.fromRGB(255, 255, 255); buyBtn.TextSize = 11; buyBtn.Text = "BUY"
		...
		giftBtn.Size = UDim2.new(0.48, 0, 1, 0); giftBtn.Font = Enum.Font.GothamBold; giftBtn.TextColor3 = Color3.fromRGB(255, 255, 255); giftBtn.TextSize = 11; giftBtn.Text = "GIFT"
	...
	for _, dp in ipairs(ItemData.Products) do
		...
		local row = Instance.new("Frame", PremList)
		row.Size = UDim2.new(1, -5, 0, 90); row.BackgroundColor3 = Color3.fromRGB(22, 22, 28); Instance.new("UICorner", row).CornerRadius = UDim.new(0, 6)
		...
		local rTitle = Instance.new("TextLabel", row); rTitle.Size = UDim2.new(1, -15, 0, 18); rTitle.Position = UDim2.new(0, 10, 0, 5); rTitle.BackgroundTransparency = 1; rTitle.Font = Enum.Font.GothamBlack; rTitle.TextColor3 = Color3.fromRGB(150, 255, 150); rTitle.TextSize = 12; rTitle.TextXAlignment = Enum.TextXAlignment.Left; rTitle.Text = dp.Name
		...
		local btn = Instance.new("TextButton", btnArea); btn.Size = UDim2.new(0.5, 0, 1, 0); btn.Font = Enum.Font.GothamBold; btn.TextColor3 = Color3.fromRGB(255, 255, 255); btn.TextSize = 11; btn.Text = "BUY"
	...
	task.spawn(function()
		while true do
			...
			if currentShopData then
				...
				-- Background check for VIP cooldown
