...
-- @ScriptType: ModuleScript (mobile)
local ProfileTab = {}
...
function ProfileTab.Init(parentFrame, tooltipMgr)
	...
	toggleStatsBtn.Size = UDim2.new(0.8, 0, 0, 35); toggleStatsBtn.BackgroundColor3 = Color3.fromRGB(30, 30, 40); toggleStatsBtn.Font = Enum.Font.GothamBold; toggleStatsBtn.TextColor3 = Color3.fromRGB(200, 200, 255); toggleStatsBtn.TextSize = 13; toggleStatsBtn.Text = "VIEW TITAN STATS"; toggleStatsBtn.LayoutOrder = 6
	...
	local asLayout = Instance.new("UIListLayout", AutoSellFrame); asLayout.FillDirection = Enum.FillDirection.Horizontal; asLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center; asLayout.Padding = UDim.new(0, 10)

	local asLabel = Instance.new("TextLabel", AutoSellFrame)
	asLabel.Size = UDim2.new(0, 55, 1, 0); asLabel.BackgroundTransparency = 1; asLabel.Font = Enum.Font.GothamBold; asLabel.TextColor3 = Color3.fromRGB(180, 180, 180); asLabel.TextSize = 10; asLabel.TextXAlignment = Enum.TextXAlignment.Right; asLabel.Text = "Auto-Sell:"
	...
	local function CreateAutoSell(rarity, color)
		...
		asBtn.Size = UDim2.new(0, 58, 1, 0); asBtn.BackgroundColor3 = Color3.fromRGB(30, 30, 35)
		asBtn.Font = Enum.Font.GothamBold; asBtn.TextColor3 = color; asBtn.TextSize = 10; asBtn.Text = "All " .. rarity
	...
	gl.CellSize = UDim2.new(0.3, -4, 0, 64)  -- 3 columns, scales to panel width
	...
	local function RefreshProfile()
		...
		local pVal = ls and ls:FindFirstChild("Prestige") and ls.Prestige.Value or 0
		local statCap = GameData.GetStatCap(pVal)

		local hasMaxStats = true
		local coreStats = {"Health", "Strength", "Defense", "Speed", "Gas", "Resolve"}
		for _, s in ipairs(coreStats) do
			if (player:GetAttribute(s) or 10) < statCap then
				hasMaxStats = false; break
			end
		end

		if cPart > 8 and hasMaxStats then prestigeBtn.Visible = true else prestigeBtn.Visible = false end
		RenderRadarChart()
		...
		for _, item in ipairs(inventoryItems) do
			...
			if count > 0 then
				...
				-- Card: rounded corners, dark base
				...
				-- Thin clean 1px border with rarity color, low opacity
				...
				-- Top accent bar: thin colored line at very top of card
				...
				-- Subtle rarity-tinted background glow (bottom gradient)
				...
				-- Count badge: top-right corner, small pill
				...
				-- Item name label: centered, compact
				...
				-- Rarity initial: bottom-left, subtle
				...
				card.MouseEnter:Connect(function() if cachedTooltipMgr then cachedTooltipMgr.Show(tTipStr) end end)
				card.MouseLeave:Connect(function() if cachedTooltipMgr then cachedTooltipMgr.Hide() end end)
				...
				if itemInfo.IsGift then
					...
					giftTag.Text = "GIFT"
					giftTag.ZIndex = 3
					-- No overlay, no click handler — fully inert
				else
					-- Actions overlay: clean dark overlay with rounded corners
					...
					-- buttonConsumed prevents child button clicks from bubbling up to card toggle
					...
					if itemInfo.Type ~= nil then 
						...
						if isEq then equipBtn.BackgroundColor3 = Color3.fromRGB(120, 45, 45); equipBtn.Text = "UNEQUIP" else equipBtn.BackgroundColor3 = Color3.fromRGB(35, 75, 35); equipBtn.Text = "EQUIP" end
					...
					elseif itemInfo.Action ~= nil then 
						equipBtn.BackgroundColor3 = Color3.fromRGB(90, 40, 110); equipBtn.Text = "USE"
					...
					card.MouseButton1Click:Connect(function()
						-- If a child button just fired, eat this event and reset the flag
		...
		task.delay(0.05, function() InvGrid.CanvasSize = UDim2.new(0, 0, 0, math.ceil((layoutOrderCounter - 1) / 3) * 74) end)
	end
